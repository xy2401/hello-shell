...
const sidebarFiles = [path.join(root, 'docs', '.vitepress', 'config.ts'), path.join(root, 'docs', '.vitepress', 'nav.mjs')].filter(fs.existsSync);
const sidebar = sidebarFiles.map(file => fs.readFileSync(file, 'utf8')).join('\n');
const failures = [];
