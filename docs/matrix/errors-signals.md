...
`setEExitCode` 的测量则换个角度：子 PowerShell 进程里设 `$ErrorActionPreference='Stop'`，未捕获的终止性错误使子进程以非零码退出，父进程从 `$LASTEXITCODE` 读到 1（快照 `setEExitCode=1`）。全局变量 `$ErrorActionPreference` 与逐 cmdlet 的 `-ErrorAction` 的关系、`$?` 与 `$LASTEXITCODE` 的分工，见 [PowerShell 分卷 pitfalls](/products/powershell/pitfalls)。`demos/powershell5/07_errors.ps1.out.txt`、`demos/powershell7/07_errors.ps1.out.txt` 已入库，四行与 Linux 侧 pwsh 快照逐字相同（ps5 版子进程用 `powershell`、ps7 版用 `pwsh`）。
...
Ctrl+C（SIGINT）与 SIGTERM 会中断前台脚本，POSIX shell 可用 `trap` 安装处理器、Windows 侧由控制台事件承载——本站不安排信号实验，因此没有对应快照；各 shell 的 trap/事件写法见分卷 pitfalls 页（如 [bash pitfalls](/products/bash/pitfalls)）。
...
- [统一任务实验总览 · 任务 07](/playground/#任务-04-控制流#任务-07-错误处理)
