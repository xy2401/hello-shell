...
任务 02（[实验总览](/matrix/experiment/#任务-04-控制流#任务-02-变量与引号)）用四个验证点钉住这条流水线：`value`（带空格赋值）、`wordCount`（分词）、`interpolated`（插值）、`starLiteral`（glob 字面量）。
...
- [统一任务实验总览 · 任务 02](/matrix/experiment/#任务-04-控制流#任务-02-变量与引号)
