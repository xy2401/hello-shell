...
| 维度 | 回答的问题 | 专页 |
| --- | --- | --- |
| 变量与引用 | 值怎么存、怎么插值、怎么分词、`*` 会不会变成 glob | [变量与引号](/concepts/quoting-variables) |
| 控制流 | 条件怎么判断、循环怎么写、目录怎么遍历 | [控制流](/concepts/control-flow) |
| 函数与管道 | 函数怎么「返回值」（值还是退出码）、管道里流的是文本还是对象 | [函数与管道](/concepts/functions-pipes) |
| 错误与信号 | 失败之后发生什么：退出码、终止性错误还是异常 | [错误与信号](/concepts/errors-signals) |
...
- 按维度读：[变量与引号](/concepts/quoting-variables) → [控制流](/concepts/control-flow) → [函数与管道](/concepts/functions-pipes) → [错误与信号](/concepts/errors-signals)
