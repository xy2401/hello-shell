...
**规避**：展开一律加双引号；确需分词时像 demo 一样显式写出来并加注释。通用规则见 [变量与引用](/fundamentals/quoting-variables)。
...
**规避**：不要假设 `set -e` 覆盖一切；关键步骤显式 `cmd || exit 1` / `cmd || fallback`，子 shell 的失败要用 `$?` 主动接住。详见 [错误与信号](/fundamentals/errors-signals)。
...
- [bash 语法骨架](./syntax)
- [bash 入参模型](./args)
- [zsh 真实陷阱](/shells/zsh/pitfalls)：分词行为与 bash 相反
- [fish 真实陷阱](/shells/fish/pitfalls)：POSIX 语法不兼容
