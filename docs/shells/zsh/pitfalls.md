...
输出 `wordCount=3`。如果按 bash 习惯写成依赖隐式分词的逻辑，在 zsh 里不会报错，只会默默把 `"a b c"` 当成一个整体处理——这类静默错误比报错更难查。反过来说，bash 里「忘加引号导致拆词」的事故（见 [bash 真实陷阱](/shells/bash/pitfalls)）在 zsh 里天然不存在。
...
输出 `firstArg=alice`、`secondArg=bob smith`。bash 版同一位置写的是 `${positional[0]}`/`${positional[1]}`（见 [bash 入参模型](/shells/bash/args)）。跨 shell 移植脚本时，下标不改会取到相邻元素且不报错。
...
改名完成后 `*.log` 已无匹配，正是 `(N)` 让校验步骤安全通过，输出 `demos/zsh/08_real_world.zsh.out.txt` 给出 `verify=ok`。bash 版同一任务靠 `[ -f "$f" ]` 逐一遍历兜底（见 [bash 语法骨架](/shells/bash/syntax)），两家写法再次分岔。
