...
**坑二：zsh 里 `status` 是保留字。** zsh 的 `status` 是 `$?` 的只读别名，不能拿来当循环变量名——所以各 shell 版本统一把字段命名为 `orderStatus`。PowerShell 与 Python 没有这个限制，直接用 CSV 列名 `status`（`$order.status`、`row["status"]`）。这类「同一个词在不同 shell 里身份不同」的坑，集中在各分卷的 pitfalls 页（如 [zsh pitfalls](/shells/zsh/pitfalls)）。

## glob 遍历：同一形态，不同实现

`loopFiles=3` 的遍历写法：bash/zsh/fish 都用 glob（`for f in /fixtures/data/*`），PowerShell 用 `Get-ChildItem`，Python 用 `os.listdir`。glob 在控制流里的角色（展开成词表供循环消费）与它在[变量与引号](/fundamentals/quoting-variables)里的行为是同一套机制。
...
- [统一任务实验总览 · 任务 04](/labs/#任务-04-控制流)
- [变量与引号](/fundamentals/quoting-variables)（glob 展开规则）、[函数与管道](/fundamentals/functions-pipes)（管道子 shell 与对象管道）
- [Shell vs Python](/compare/shell-vs-python)：当过滤逻辑复杂到要维护状态时，为什么该换 Python
