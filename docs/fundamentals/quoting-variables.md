...
> 本页结论：变量模型可以统一成一条流水线——赋值 → 存储 → 引用（插值或字面量）→ 分词 → glob。八个运行体在这条流水线上各有默认值：bash 对未加引号的展开**默认分词**，zsh **默认不分词**（需 `${=var}` 显式开启）；PowerShell 单引号是字面量、双引号才插值；Python 字符串根本不经过分词和 glob。只要引用时带引号（或语言本身无展开机制），含 `*` 的值就永远是字面量。任务 02 在五个 Linux 运行体上的输出完全一致，`demos/cmd`、`demos/powershell5`、`demos/powershell7` 的 Windows 快照待首次采集。
...
任务 02（[实验总览](/labs/#任务-02-变量与引号)）用四个验证点钉住这条流水线：`value`（带空格赋值）、`wordCount`（分词）、`interpolated`（插值）、`starLiteral`（glob 字面量）。

## 证据：任务 02 输出五体一致

以下快照逐字摘自 `demos/`（五个 Linux 运行体输出完全相同，这里各引一份关键行）：
...
两份输出都是 `wordCount=3`（见上方快照），但写 bash 脚本时要防「意外分词」，写 zsh 脚本时要记得「想要分词得显式开」。fish 的变量天然是列表，不存在标量分词问题；cmd 用 `for %%w in (!WORDS!)` 做词法切分（Windows 快照待首次采集）。横向对照见[引号矩阵](/matrix/quoting-matrix)。
...
- cmd：`set "STAR=a*b*c"` 后 `!STAR!` 保持字面，cmd 本身不做 glob 展开（Windows 快照待首次采集）。
...
| `demos/cmd/02_variables_quoting.bat` | 源码在库，Windows 快照待首次采集 |
| `demos/powershell5/02_variables_quoting.ps1`、`demos/powershell7/02_variables_quoting.ps1` | 源码在库，Windows 快照待首次采集 |

## 延伸阅读

- [引号矩阵](/matrix/quoting-matrix)、[通配矩阵](/matrix/globbing-matrix)
- [统一任务实验总览 · 任务 02](/labs/#任务-02-变量与引号)
- 各 shell 引号细节：[bash](/shells/bash/syntax)、[zsh](/shells/zsh/syntax)、[fish](/shells/fish/syntax)、[cmd](/shells/cmd/syntax)、[PowerShell](/shells/powershell/syntax)
