...
其中 `<shell>` 为 `bash`/`zsh`/`fish`/`pwsh`/`python`/`cmd`/`powershell5`/`powershell7`，`<ext>` 为对应脚本扩展名（`.sh`/`.zsh`/`.fish`/`.ps1`/`.py`/`.bat`）。骨架解读见 [统一语义骨架](/fundamentals/)，证据的采信规则见 [证据政策](/reference/evidence-policy)。
...
**目标**：验证带空格赋值、分词计数、插值、含 `*` 值保持字面四个点。模型解析见 [变量与引号](/fundamentals/quoting-variables)。
...
**目标**：`for` 求和、逐行读 CSV 过滤、glob 遍历计数。模型与坑位解析见 [控制流](/fundamentals/control-flow)。
...
**目标**：区分值通道与退出码通道——`functionResult=42` 走值通道，`exitCodeReturn=7` 走退出码通道，`afterCall=outer` 验证局部作用域。解析见 [函数与管道](/fundamentals/functions-pipes)。
...
**目标**：glob 列举、通配计数、模式数行、按 CSV 列分组统计——同一条统计，文本管道、对象管道、显式代码三种写法。解析见 [函数与管道](/fundamentals/functions-pipes)。
...
**目标**：捕获一次失败并继续执行，测量「失败即停」语义对应的退出码。光谱模型见 [错误与信号](/fundamentals/errors-signals)。
...
- [统一语义骨架](/fundamentals/)：五个维度与 9 任务的映射
...
- [Shell vs Python](/compare/shell-vs-python)、[证据政策](/reference/evidence-policy)
